...
namespace App\Http\Controllers\MIS\Road;
...
class RoadMISController extends Controller
{
    public function __construct()
    {
        DB::enableQueryLog();
        Log::info("RoadMISController");
    ...
    public function searchCdWorks(Request $request)
    {
        try {
            if ($request->header('X-CSRF-TOKEN')) {
                ...
                $baseQuery = DB::table('asset_road_cdwork_details')
                    ...
                    ->leftJoin('asset_road_details', 'asset_road_cdwork_details.rd_system_id', '=', 'asset_road_details.rd_system_id')
                ...
                $culvertDetails = $baseQuery->get();

                if ($culvertDetails) {
                    if ($culvertDetails->count() == 0) {
                        return response()->json([
                            'status' => 200,
                            'message' => 'Data not available!!',
                            'result' => $culvertDetails
                        ]);
                    } else {
                        return response()->json([
                            'status' => 200,
    ...
    public function searchBridge(Request $request)
    {
        try {
            if ($request->header('X-CSRF-TOKEN')) {
                $roadID = $request->input('road');
                ...
                if ($roadDetails) {
                    ...
                    return response()->json([
                        'status' => 204,
                        'message' => 'Bridge details not available!',
                        'result' => null
                    ]);
                }
            } else {
    ...
    public function searchPCI(Request $request)
    {
        try {
            if ($request->header('X-CSRF-TOKEN')) {
                $roadID = $request->input('road');
                ...
                $baseQuery = DB::table('asset_road_pavement_condition_indexes')
                    ->select(
                        'asset_road_pavement_condition_indexes.*',
                        'asset_road_details.rd_name',
                    )
                    ->leftJoin('asset_road_details', 'asset_road_pavement_condition_indexes.rd_system_id', '=', 'asset_road_details.rd_system_id')
                ...
                $PCIDetails = $baseQuery->get();

                if ($PCIDetails) {
                    if ($PCIDetails->count() == 0) {
                        return response()->json([
                            'status' => 200,
                            'message' => 'Data not available!',
                            'result' => $PCIDetails
    ...
    public function searchProtectionWall(Request $request)
    {
        try {
            if ($request->header('X-CSRF-TOKEN')) {
                $roadID = $request->input('road');
                ...
                $baseQuery = DB::table('asset_protection_wall_details')
                    ...
                    ->leftJoin('asset_road_details', 'asset_protection_wall_details.rd_system_id', '=', 'asset_road_details.rd_system_id')
    ...
    public function searchSurfaceType(Request $request)
    {
        try {
            if ($request->header('X-CSRF-TOKEN')) {
                $roadID = $request->input('road');
                ...
                $baseQuery = DB::table('asset_road_surface_type_details')
                    ->select(
                        'asset_road_surface_type_details.*',
                    ...
                    ->leftJoin('asset_road_details', 'asset_road_surface_type_details.rd_system_id', '=', 'asset_road_details.rd_system_id')
                ...
                if ($surfaceTypeDetails) {
                    if ($surfaceTypeDetails->count() == 0) {
                        return response()->json([
                            'status' => 200,
                            'message' => 'Data not available!',
                            'result' => $surfaceTypeDetails
                        ]);
                    } else {
                        return response()->json([
                            'status' => 200,
    ...
    {
        Log::info("Coming to the searchHabitation");
        try {
            if ($request->header('X-CSRF-TOKEN')) {
                $roadID = $request->input('road');
                ...
                $baseQuery = DB::table('asset_road_habitation_details')
                    ...
                    ->leftJoin('asset_road_details', 'asset_road_habitation_details.rd_system_id', '=', 'asset_road_details.rd_system_id')
                    ...
                    ->whereIn('asset_road_habitation_details.rd_system_id', function ($query) {
                        $query->select(DB::raw('distinct(ard.rd_system_id)'))
                            ->from('asset_road_details as ard')
                            ->where('ard.road_type', 'SR');
                    })
                ...
                if ($habitationDetails) {
                    if ($habitationDetails->count() == 0) {
                        return response()->json([
                            'status' => 200,
                            'message' => 'Data not available!',
                            'result' => $habitationDetails,
                            'facilities' => $habitationFacilities
                        ]);
                    } else {
                        return response()->json([
                            'status' => 200,
