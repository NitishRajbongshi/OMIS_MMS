...
class OrganisationalStructureController extends Controller
{
    ...
    public function getENCTreeData()
    {
        try {
            ...
            $count_zn = DB::table('asset_master_zones AS zn')
                        ->select(DB::raw('count(zn.zone_cd) as total'))
                        ->where("zn.dept_cd", "=", "14")
                        ->get();
            ...
            $count_zn_pep = DB::table('asset_master_zones AS zn')
                            ->select(DB::raw('count(zn.zone_cd) as total'))
                            ->where("zn.dept_cd", "=", "19")
                            ->get();
            
            $count_zn_pnd = DB::table('asset_master_zones AS zn')
                            ->select(DB::raw('count(zn.zone_cd) as total'))
                            ->where("zn.dept_cd", "=", "20")
                            ->get();

            $count_crl = DB::table('asset_master_circles AS crl')
                        ->select(DB::raw('count(crl.circle_cd) as total'))
            ...
            $count_crl_pep = DB::table('asset_master_circles AS crl')
                            ->select(DB::raw('count(crl.circle_cd) as total'))
                            ->where("crl.dept_cd", "=", "19")
                            ->get();

            $count_crl_pnd = DB::table('asset_master_circles AS crl')
                            ->select(DB::raw('count(crl.circle_cd) as total'))
                            ->where("crl.dept_cd", "=", "20")
                            ->get();

            $count_div = DB::table('asset_master_divisions AS dv')
                            ->select(DB::raw('count(dv.division_cd) as total'))
                            ->where("dv.dept_cd", "=", "14")
                            ->get();
            ...
            $count_div_nh = DB::table('asset_master_divisions AS dv')
                            ...
                            ->where("dv.dept_cd", "=", "3")
                            ->get();
            $count_div_pep = DB::table('asset_master_divisions AS dv')
                            ->select(DB::raw('count(dv.division_cd) as total'))
                            ->where("dv.dept_cd", "=", "19")
                            ->get();
            $count_div_pnd = DB::table('asset_master_divisions AS dv')
                            ->select(DB::raw('count(dv.division_cd) as total'))
                            ->where("dv.dept_cd", "=", "20")
                            ->get();
            
            $count_sdiv = DB::table('asset_master_sub_divisions AS sdv')
                            ->select(DB::raw('count(sdv.sub_div_cd) as total'))
            ...
            $count_sdiv_pep = DB::table('asset_master_sub_divisions AS sdv')
                            ->select(DB::raw('count(sdv.sub_div_cd) as total'))
                            ->where("sdv.dept_cd", "=", "19")
                            ->get();

            $count_sdiv_pnd = DB::table('asset_master_sub_divisions AS sdv')
                            ->select(DB::raw('count(sdv.sub_div_cd) as total'))
                            ->where("sdv.dept_cd", "=", "20")
                            ->get();

            $count_zn_for_hq = DB::table('asset_master_zones AS zn')
                            ->select(DB::raw('count(zn.zone_cd) as total'),"zn.dept_cd")
                            ->groupBy("zn.dept_cd")
                            ->get();


            $count_crl_for_hq = DB::table('asset_master_circles AS crl')
                                ->select(DB::raw('count(crl.circle_cd) as total'),"crl.dept_cd")
                                ->groupBy("crl.dept_cd")
                                ->get();

            $count_div_for_hq = DB::table('asset_master_divisions AS dv')
                                ->select(DB::raw('count(dv.division_cd) as total'),"dv.dept_cd")
                                ->groupBy("dv.dept_cd")
                                ->get();

            $count_sdiv_for_hq = DB::table('asset_master_sub_divisions AS sdv')
                                ->select(DB::raw('count(sdv.sub_div_cd) as total'),"sdv.dept_cd")
                                ->groupBy("sdv.dept_cd")
                                ->get();
    ...
    public function getCEUserDataAndSEOffficeData(Request $request)
    {
        $dept_id  = $request->dept_id;
        DB::enableQueryLog();
        $hq_user_data = DB::table('office_details AS ofd')
    ...
    public function getAdditionalOfficeChargeDetails(Request $request)
    {
        $addlOfficeDetails= [];
        $ret_value = response()->json(["status" => False,
                                    "additional_ofis_data"=>json_encode($addlOfficeDetails)
                                ]);
        try
        {
            DB::enableQueryLog();
            $additional_office_details = DB::table('users AS u')
                                        ->select("u.name", "u.email", "u.additional_office_details")
                                        ->whereNotNull("u.additional_office_details")
                                        ->WhereRaw("additional_office_details->>'office_details' <> '[]'")
                                        ->get();
            $query = DB::getQueryLog();
            Log::info($query);
            if ($additional_office_details)
            {
                foreach ($additional_office_details as $item)
                {
                    $addlOfs = json_decode ($item->additional_office_details)->office_details;
                    foreach ($addlOfs as $jsonData)
                    {
                        if($jsonData->status == "A")
                        {
                            $ofs_cd = $jsonData->office;
                            $ofs_zone_etc_dtls = DB::table('office_details AS ofd')
                                                    ->select(
                                                        "ofd.zone_cd",
                                                        "ofd.circle_cd",
                                                        "ofd.division_cd",
                                                    ...
                                                    ->where("ofd.id", "=", $ofs_cd)->get()->first();
                            if($ofs_zone_etc_dtls)
                            {
                                $jsonData->zone_cd = $ofs_zone_etc_dtls->zone_cd;
                                $jsonData->circle_cd = $ofs_zone_etc_dtls->circle_cd;
                                $jsonData->division_cd = $ofs_zone_etc_dtls->division_cd;
                                $jsonData->sub_division_cd = $ofs_zone_etc_dtls->sub_division_cd;
                            }
                            $jsonData->userName = $item->name;
                            $jsonData->userEmail = $item->email;
                            array_push($addlOfficeDetails, $jsonData);
                        }
                    }
                }
            }
            if(count($addlOfficeDetails) >0 )
                $ret_value = response()->json(["status" => True,
                        "additional_ofis_data"=>$addlOfficeDetails
                    ]);
        }
        catch (Exception $e) 
        {
            Log::error("Error in getAdditionalOfficeChargeDetails function: " . $e->getMessage());
        }
